...
// node_modules/folder-hash/index.js
var require_folder_hash = __commonJS({
  "node_modules/folder-hash/index.js"(exports, module2) {
    ...
    function prep(fs2) {
      function hashElement(name, dir, options, callback) {
        ...
        return parseParameters(arguments).then(({ basename, dir: dir2, options: options2 }) => {
          options2.skipMatching = true;
          return fs2.promises.lstat(path.join(dir2, basename)).then((stats) => {
            stats.name = basename;
            return stats;
      ...
      function hashFolderPromise(name, dir, options, isRootElement = false) {
        ...
        return fs2.promises.readdir(folderPath, { withFileTypes: true }).then((files) => {
          const children = files.sort((a, b) => a.name.localeCompare(b.name)).map((child) => {
            return hashElementPromise(child, folderPath, options);
          });
          return Promise.all(children).then((children2) => {
            if (ignoreBasenameOnce)
              options.ignoreBasenameOnce = true;
            const hash = new HashedFolder(name, children2.filter(notUndefined), options, isRootElement);
            return hash;
          });
        });
      }
      function hashFilePromise(name, dir, options, isRootElement = false) {
        ...
        return new Promise((resolve, reject) => {
          try {
            ...
            const f = fs2.createReadStream(filePath);
      ...
      async function hashSymLinkPromise(name, dir, options, isRootElement = false) {
        const target = await fs2.promises.readlink(path.join(dir, name));
      ...
      async function symLinkResolve(name, dir, target, options, isRootElement) {
        ...
        try {
          const stats = await fs2.promises.stat(path.join(dir, name));
          stats.name = name;
      ...
      function ignore(name, path2, rules) {
        if (rules.exclude) {
          if (rules.matchBasename && rules.exclude(name)) {
            log.match(`exclude basename '${name}'`);
            return true;
          } else if (rules.matchPath && rules.exclude(path2)) {
            log.match(`exclude path '${path2}'`);
            return true;
          }
        }
        if (rules.include) {
          if (rules.matchBasename && rules.include(name)) {
            ...
            return false;
          } else if (rules.matchPath && rules.include(path2)) {
            log.match(`include path '${path2}'`);
            return false;
          } else {
            log.match(`include rule failed for path '${path2}'`);
            return true;
          }
        }
        log.match(`Will not ignore unmatched '${path2}'`);
        return false;
      }
      return hashElement;
    }
    function parseParameters(args) {
      let basename = args[0], dir = args[1], options_ = args[2];
...
${padding}]`;
    ...
    module2.exports = {
      ...
      hashElement: prep(require_graceful_fs()),
      prep,
      parseParameters
    };
  }
...
function start() {
  return __async(this, null, function* () {
    try {
      const dirs = import_core.default.getMultilineInput("parts", {
        required: true
      });
      const parts = [];
      for (const dir of dirs) {
        ...
        const { hash } = yield import_folder_hash.default.hashElement(dir);
        parts.push(hash);
      }
      import_core.default.setOutput("hash", parts.join(""));
    } catch (e) {
